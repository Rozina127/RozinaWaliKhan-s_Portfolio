...
// Generate CSS string for chart theming - sanitized to prevent XSS
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const cssContent = React.useMemo(() => generateChartCSS(id, config), [id, config]);
...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const configLabel = !labelKey && typeof label === "string" ? config[label as keyof typeof config] : null;
      const value = configLabel?.label || (typeof label === "string" ? label : itemConfig?.label);
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  } else if (payloadPayload && Object.prototype.hasOwnProperty.call(payloadPayload, key)) {
